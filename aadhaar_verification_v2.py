...
def retried_aadhaar_verification(driver, captcha_not_detected):
    captcha_not_detected_2 = []
    for aadhaar in captcha_not_detected:
        ...
        aadhar_number = aadhaar['aadhar_number']#input("Enter Your Aadhar Number : ")
        print(f"Enter the Aadhaar number for {aadhaar['Full Name']} is {aadhar_number}")
            ...
            try:
                if len(captcha) == 6:
                    ...
                    if driver.find_element(By.XPATH, '//*[@id="root"]/div/div[3]/div/div/span[2]'):
                        ...
                        os.remove(os.path.join(Images_invalid_dir, f"{aadhaar['S. No']}_{aadhaar['Full Name']}.png"))
...
#                     print(len(captcha)) 
                    ...
                    attempt, message = i, "Fail"
            if i==19: 
                captcha_not_detected_2.append({"S. No":aadhaar['S. No'], "Full Name":aadhaar['Full Name'], "aadhar_number": aadhar_number})
...
#                     print(len(captcha)) 
                    ...
                    attempt, message = i, "Fail"
            if i==19: 
                captcha_not_detected.append({"S. No":aadhaar['S. No'], "Full Name":aadhaar['Full Name'], "aadhar_number": aadhar_number})
                print("Invalid or captcha Issue")
        log = f"{datetime.now()} ({aadhaar['S. No']}) ({aadhaar['Full Name']}) ({aadhar_number}) attempt of captcha ({attempt}) Result ({message}) \n"
        print(log)
        with open(os.path.abspath("log for aadhar v2.txt"), 'a') as f:
        ...
        print("Get Back to Home Page https://myaadhaar.uidai.gov.in \n")
        time.sleep(4)
    print(captcha_not_detected)
    print("LOADING [INFO] Analyse the Not chaptered Aadhaar............")
    time.sleep(10)

    if captcha_not_detected:
        captcha_not_detected_2 = retried_aadhaar_verification(driver, captcha_not_detected)
        print(captcha_not_detected_2)

    print("LOADING [INFO] Completed Aadhaar Verification............")
    time.sleep(10)
...
if __name__ == "__main__":
    ...
    win.destroy()
    print(filename)
    aadhaar_details = pd.read_excel(filename)[0:5]
